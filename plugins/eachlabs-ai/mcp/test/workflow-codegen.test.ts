...
test("Go codegen is formatted by gofmt and omits deprecated version", (t) => {
  const result = generateIntegrationCode({
    model: "live-model",
    schema: MODEL_SCHEMA,
    language: "go",
  ...
  assert.match(result.code, /package main/);
  assert.match(result.code, /EACH_API_KEY/);
  assert.doesNotMatch(result.code, /"version"/);
  const formatted = spawnSync("gofmt", { input: result.code });
  if (formatted.error && (formatted.error as NodeJS.ErrnoException).code === "ENOENT") {
    t.skip("gofmt is not installed in this environment");
    return;
  }
  assert.equal(formatted.status, 0, formatted.stderr?.toString());
});
